...
function Skills() {
	...
	const shouldPullToResults = connectedCount > 0 && connectedCount <= 3;
	const resultItems = React.useMemo(
		() => [
			...leftMatches.map((name) => ({ name, side: "Technology" })),
			...rightMatches.map((name) => ({ name, side: "Soft Skill" })),
		],
	...
	}, [resultItems, shouldPullToResults]);
	...
	return (
		...
		>
			...
			<div className="w-full px-8 mt-8 flex items-center justify-center gap-3">
				...
				<div
					className="mx-auto w-3/4 max-w-6xl h-[78vh] min-h-130 flex items-center justify-center flex-col gap-6"
					id="search-area"
				>
					<div
			...
			<style jsx>{`
				.connector-path {
					...
					opacity: 1;
					filter:
						drop-shadow(0 0 3px rgba(186, 230, 253, 0.95))
						drop-shadow(0 0 11px rgba(56, 189, 248, 0.72));
